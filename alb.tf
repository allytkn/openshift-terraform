...
resource "aws_lb_target_group_attachment" "attachment_master2_alb" {
  target_group_arn = "${aws_lb_target_group.group_master_alb.arn}"
  target_id        = "${aws_instance.master2.id}"
  port             = 8443
}
resource "aws_lb_target_group_attachment" "attachment_master3_alb" {
  target_group_arn = "${aws_lb_target_group.group_master_alb.arn}"
  target_id        = "${aws_instance.master3.id}"
  port             = 8443
}
resource "aws_lb_target_group_attachment" "attachment_infra1_alb" {
  target_group_arn = "${aws_lb_target_group.group_infra_alb.arn}"
  target_id        = "${aws_instance.infra1.id}"
  port             = 80
}
resource "aws_lb_target_group_attachment" "attachment_infra2_alb" {
  target_group_arn = "${aws_lb_target_group.group_infra_alb.arn}"
  target_id        = "${aws_instance.infra2.id}"
  port             = 80
}
resource "aws_lb_target_group_attachment" "attachment_infra3_alb" {
  target_group_arn = "${aws_lb_target_group.group_infra_alb.arn}"
